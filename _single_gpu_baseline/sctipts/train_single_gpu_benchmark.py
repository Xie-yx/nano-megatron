...
def parse_args() -> tuple[TrainConfig, GPTConfig]:
    parser = argparse.ArgumentParser(description="Single-GPU GPT trainer for benchmark")
    ...
    parser.add_argument("--eval-iters", type=int, default=20, help="Number of batches per evaluation")
    parser.add_argument("--log-interval", type=int, default=10, help="Logging interval in steps")
    parser.add_argument("--save-interval", type=int, default=100, help="Checkpoint interval in steps")
    ...
    parser.add_argument(
        "--vocab-size",
        type=int,
        default=GPT2_TOKENIZER_SPEC.vocab_size,
        help="Vocabulary size. Keep aligned with the tokenizer used by prepare.py",
    )
    ...
    parser.add_argument("--ffn-hidden-size", type=int, default=1024, help="MLP intermediate size")
    parser.add_argument("--num-attention-heads", type=int, default=4, help="Number of attention heads")
    parser.add_argument("--max-seq-len", type=int, default=128, help="Sequence length / block size")
    ...
    parser.add_argument("--use-bias", action="store_true", help="Enable bias in linear layers")
...
def load_token_files(data_dir: str) -> tuple[np.memmap, np.memmap]:
    ...
    if not train_path.exists() or not val_path.exists():
        raise FileNotFoundError(f"Could not find preprocessed token files. Run {data_path / 'prepare.py'} first.")
    train_tokens = np.memmap(train_path, dtype=GPT2_TOKENIZER_SPEC.token_dtype, mode="r")
    val_tokens = np.memmap(val_path, dtype=GPT2_TOKENIZER_SPEC.token_dtype, mode="r")
    if train_tokens.size == 0 or val_tokens.size == 0:
        raise ValueError("train.bin or val.bin is empty.")
...
def get_batch(token_ids: np.memmap, batch_size: int, block_size: int, device: str) -> tuple[torch.Tensor, torch.Tensor]:
    if token_ids.shape[0] <= block_size + 1:
        raise ValueError("Token sequence length must be larger than max_seq_len.")
...
def estimate_loss(
    ...
    for split_name, split_tokens_tensor in (("train", train_tokens), ("val", val_tokens)):
        ...
        for index in range(train_config.eval_iters):
            ...
            split_losses[index] = F.cross_entropy(logits.view(-1, logits.size(-1)), labels.view(-1)).item()
...
def main() -> None:
    ...
    if train_config.max_steps <= start_step:
        raise ValueError(f"max_steps ({train_config.max_steps}) must be greater than resume step ({start_step}).")
    ...
    use_cuda = train_config.device.startswith("cuda")
    if use_cuda:
        torch.cuda.reset_peak_memory_stats(device=train_config.device)

    model.train()
    for step in range(start_step + 1, train_config.max_steps + 1):
        if use_cuda:
            torch.cuda.synchronize(device=train_config.device)
        ...
        if use_cuda:
            max_allocated_mb = torch.cuda.max_memory_allocated(device=train_config.device) / (1024 ** 2)
            max_reserved_mb = torch.cuda.max_memory_reserved(device=train_config.device) / (1024 ** 2)

        if step % train_config.log_interval == 0 or step == 1:
            msg = f"step {step:06d} | train loss {loss.item():.4f} | step time {elapsed_ms:.2f} ms"
        ...
        if train_config.eval_interval > 0 and step % train_config.eval_interval == 0:
            ...
            print(f"step {step:06d} | eval train loss {losses['train']:.4f} | eval val loss {losses['val']:.4f}")
            append_csv_log(
                csv_path,
                event="eval",
                step=step,
    ...
    if use_cuda:
        final_allocated_mb = torch.cuda.max_memory_allocated(device=train_config.device) / (1024 ** 2)
        final_reserved_mb = torch.cuda.max_memory_reserved(device=train_config.device) / (1024 ** 2)
        print(
            f"BENCHMARK_SUMMARY mode=single max_allocated_mb={final_allocated_mb:.4f} "
            f"max_reserved_mb={final_reserved_mb:.4f}"
        )
